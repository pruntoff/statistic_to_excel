...
df.to_excel('{}.xlsx'.format(args.folder))

df = mf.clear_df(df)

if args.det_bank and (args.det_bank in mf.set_df_field(df, 'info_system')):
    mf.bank_detailed(df.loc[df['info_system'] == args.det_bank], args)
    print('Выполнено')
    sys.exit(0)
elif args.det_bank:
    print('Указанный вами банк {} не найден. Необходимо точное наименование'.format(args.det_bank))
    sys.exit(0)
else:
    pass
